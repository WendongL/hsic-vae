...
class BenchmarkDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path, dataset_name, variant, mode, standardise=False, imagenet_normalise=False,
        ...
        if dataset_name == 'dsprites':
            ...
            # color is given is hue values, so it has an order although at 
            # 10 points probably is is better to simply defined them as categorical variables 
            # since the difference between hue color is too bigs
            ...
            # self._factor_discrete = [False, False, False]
        ...
        if dataset_name == 'cars3d':
            self._dataset_images = np.transpose(self._dataset_images, (0, 3, 1, 2))
    ...
    def __getitem__(self, idx: int, normalize: bool = True):
        ...
        # print(f'image1: {image.shape}')
        if self.transform is not None:
            # print('transpseeee')
            # TODO: is this the best place to do the transpose?? the transform already does a transpose
            image = np.transpose(image, (1, 2, 0)) 
            # print(f'image1.5: {image.shape}')
            # print(f'image: min, mean, max : {image.min()}, {image.mean()}, {image.max()} ')
