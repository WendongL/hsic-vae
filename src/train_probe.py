...
def run(params):
    ...
    elif 'resnet' in params.model_type:
        params.representation_dataset_path = params.resnet_representation_dataset_path
    ...
    if params.cached_representations and ('resnet' in params.model_type or 'vae' in params.model_type ):
        ...
        if params.model_type == 'raw_data':
            ...
            if os.path.exists(model_file):
                print(f'loading almost unoform matrix from {model_file}')
    ...
    #             time2 = time.time()
    #         continue
    #     print(f'prepare loading: {time2 - time1}')
    ...
    if params.probe.type == 'MLP_reg':
        ...
        probe = probe.to(device)
    elif params.probe.type == 'MLP_reg_cl_ind':
    ...
    # compute standard disentanglement metrics: DCI
    # dci = compute_disentanglement_metric(model, 
    ...
    # dci = get_probe_dci(probe=probe, 
    #             dataloader_train=dataloader_train, 
    #             dataloader_val=dataloader_val, 
    #             dataloader_test=dataloader_test, 
    #             method=params.probe.type,
    ...
    # pdb.set_trace()
    # img, targets = next(iter(dataloader_train))
    # img_val, targets_val = next(iter(dataloader_val))
    # probe.probe.ind_pred[0].weight.sum()
