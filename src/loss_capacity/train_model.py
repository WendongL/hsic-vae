...
from loss_capacity.models import ConvNet, NoisyLabels, LinearMixLabels, RawData
from copy import deepcopy
import time
import numpy as np
import pdb
import sys
sys.path.insert(1, '/home/anicolicioiu/projects/sage/')
import sage


def compute_probe_loss(output, target, factor_sizes, factor_discrete, cross_ent_mult=0):
    idx_factor_start = 0
    eps = 0.00001
    ...
    for i in range(len(factor_sizes)):
        if factor_discrete[i]:
            ...
            # divide cross entropy loss by K / 2
            # cross_ent = cross_ent * 2.0 / one_hot.shape[1]
...
def train_test_probe(model, dataloader_train, dataloader_val, dataloader_test,
        seed = 0, lr = 0.001, weight_decay=0.0, optim_steps = [50], epochs = 100, log_interval = 50, save_model = True,
        train_name='model', 
    ...
    # rs_old = MetricsPytorchL2(dataloader_val)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(dataloader_train):
            ...
            optimizer.zero_grad()
            output = model(data)
            # h1 = np.histogram(target[:,0].cpu().numpy(), bins=183)
            # h2 = np.histogram(target[:,1].cpu().numpy(), bins=183)
            # h3 = np.histogram(target[:,2].cpu().numpy(), bins=183)

            # model.probe.ind_pred[2].weight[0]
            # eps = 0.00001
            # target_index = target[:,2] * (183 - 1) + eps # target is already normalised
            # one_hot = output[:, 2:]
            # loss_cross = F.cross_entropy(one_hot, target_index.type(torch.int64))
            # loss = loss_cross
            ...
            if (batch_idx == 0) or ((batch_idx + 1) % (20 * log_interval) == 0):
                # print(f'data.mean(0): {data.mean(0)}')
                # print(f'data.std(0): {data.std(0)}')
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(dataloader_val):
                ...
                # rs_old.acum_stats(output, target)
        ...
        # val_scores_old = rs_old.get_scores()['rsquared']
        print(f'[Epoch {epoch}] Scores: {scores}')
        tb_writer.add_scalar(f'{train_name}-rsquared_acc/val', scores['rsquared'], global_step_val)
        # tb_writer.add_scalar(f'{train_name}-rsquared/val', val_scores_old, global_step_val)
    ...
    # tb_writer.add_scalar(f'{train_name}-rsquared/test', test_final_scores_old, global_step_val)
    ...
    #TODO: de returnat per factors scores
    # test_scores = np.array([scores[f'rsquared_{name}'] for name in factor_names ])
    # test_scores = np.array([scores[f'rsquared_{name}'] for name in factor_names ])

    # return best_model, test_loss, test_final_scores, val_scores
    # return best_model, test_loss, test_final_scores, val_scores
...
def evaluate_probe(model, dataloader_test,
    ...
    # feats = model.model.encode(inputs.to(device))
    ...
    #     dci_scores["sage_disentanglement"] = 0
    #     dci_scores["sage_completeness"] = 0
    #     dci_scores["sage_dci"] = 0
    ...
    if compute_dci:
        ...
        if group_pixels:
            num_marginal_samples = 16 #16 # 512
        else:
            num_marginal_samples = 32 # 32 512
        # sage_thresh = 0.3 #05
        # sage_thresh = 0.2 #05
        sage_thresh = 0.1 #05
        # sage_thresh = 0.05 #05
        ...
        for i in range(len(factor_sizes)):
            ...
            if factor_discrete[i]:
                ...
                # model_factor_i = lambda x: model.forward_factors(x)[i]
                # model_factor_i = lambda x: repr_probe.forward_factors(
                #         torch.Tensor(x).to(device))[i].detach().cpu().numpy()
                model_factor_i = lambda x: repr_probe.forward_factors(x)[i].detach().cpu().numpy()
                if group_pixels:
                    # imputer = sage.GroupedMarginalImputer(model_factor_i, all_feats[:num_marginal_samples], groups)
                    imputer = sage.GroupedMarginalImputerPytorch(model_factor_i, 
                            torch.Tensor(all_feats[:num_marginal_samples]).to(device), 
                            groups)

                else:
                    # imputer = sage.MarginalImputer(model_factor_i, all_feats[:num_marginal_samples])
                ...
                sage_values = estimator(all_feats, target_factor, batch_size=128, thresh=sage_thresh,
                                verbose=True)
                # R_sage[:,i] = np.abs(sage_values.values)  # TODO: these are not normalised
...
# def evaluate_probe_cont(model, dataloader_test,
#         seed = 0,
#         log_interval = 50,
#         train_name='model', 
#         tb_writer=None,
...
#     inputs, targets = next(iter(dataloader_test))
#     len_datasplits = len(dataloader_test.dataset)
#     all_feats = np.zeros((len_datasplits, *inputs.shape[1:])).astype(np.float32)
#     all_targets = np.zeros((len_datasplits, targets.shape[1])).astype(np.float32)
#     batch_size = dataloader_test.batch_size
...
#             all_feats[batch_idx * batch_size: batch_idx * batch_size + data.shape[0]] = feats.detach().cpu().numpy()
#             all_targets[batch_idx * batch_size: batch_idx * batch_size + target.shape[0]] = target
...
#     factor_discrete = dataloader_test.dataset._factor_discrete
#     R_sage = np.zeros((all_feats.shape[1], len(factor_sizes)))
#     R_linear = np.zeros((all_feats.shape[1], len(factor_sizes)))
...
#         if True:
#             target_factor = all_targets[:,i]
#             model_factor_i = lambda x: repr_probe(torch.Tensor(x).to(device))[:,i].detach().cpu().numpy()

#             imputer = sage.MarginalImputer(model_factor_i, all_feats[:512])
#             estimator = sage.PermutationEstimator(imputer, 'mse')
#             sage_values = estimator(all_feats, target_factor, batch_size=128, thresh=0.05)
#             R_sage[:,i] = np.abs(sage_values.values)  # TODO: these are not normalised
#             R_linear[:,i] = np.abs(repr_probe.probe[0].weight[i].detach().cpu().numpy())

#             print(f'[{i}] X.mean(0): {all_feats.mean(0)}')
#             print(f'[{i}] Y.mean(0): {target_factor.mean(0)}')

#             print(f'[{i}] importance_linear: {R_linear[:,i]}')
#             print(f'[{i}] importance_sage: {R_sage[:,i]}')

#             bias = repr_probe.probe[0].bias[i].abs().detach().cpu().numpy()
#             print(f'[{i}] bias: {bias}')
...
#     dci_scores["linear_disentanglement"] = disentanglement(R_linear)
#     dci_scores["linear_completeness"] = completeness(R_linear)
#     dci_scores["linear_dci"] = (dci_scores["informativeness_test"]
#             + dci_scores["linear_disentanglement"]
#             + dci_scores["linear_completeness"]) / 3.0 
        

#     for name, score in dci_scores.items():
#         print(f'DCI: {name}: {score} ')
#         for i in range(10):
#             tb_writer.add_scalar(f'{train_name}-scores/DCI_{name}', score, model.count_parameters() + i)
