...
def train_test_random_forest(
    ...
    # we do now. we make the selection in dataloader
    ...
    ## remove this block. already reduced the data when reading the dataset
    # file_perm_train = f'./internal_files/{dataloader_train.dataset.dataset_name}_{dataloader_train.dataset.mode}_train_perm.npy'
    # if os.path.exists(file_perm_train):
    #     perm = np.load(file_perm_train)
    # else:
    #     perm = np.random.permutation(len(dataloader_train.dataset))
    #     np.save(file_perm_train, perm)
    # # select the same num_train_samples every time
    # train_h = train_h[perm][:num_train]
    # train_targets = train_targets[perm][:num_train]
