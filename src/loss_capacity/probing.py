...
class Probe(nn.Module):
    def __init__(self, model, num_factors, num_hidden_layers=1, multiplier=16):
        ...
        if self.num_hidden_layers > 0:
            # TODO: should the hidden dim be proportional to num_factors?? or should if be fixed??
            self.hidden_dim = self.multiplier * self.num_factors

            mlp_probe.append(nn.Linear(self.input_dim, self.hidden_dim))
            self.probe_at_index.append(nn.Sequential(*mlp_probe)) # TODO: should get the represenation after the relu??
        ...
        # TODO: should we use ReLU before the last linear layer in the probe?
        linear_probe = nn.Linear(self.hidden_dim, num_factors)
        # nn.init.xavier_uniform(linear_probe.weight, gain=50)

        mlp_probe.append(linear_probe)

        self.probe = nn.Sequential(*mlp_probe)
        # for i in range(self.num_hidden_layers+1):
        #     self.probe_at_index.append(nn.Sequential(*mlp_probe[:i+1]))
    ...
    def __init__(self, model, num_factors, num_hidden_layers=1, hidden_dim=0, factor_sizes=[2], 
        ...
        for i in range(num_factors):
            if self.factor_discrete[i]:
                ...
                num_outputs += 1
        # if extra_hid_params:
        #     print('WARNING: using hiddend_dim = num_outputs + multiplier * num_factors')
        #     extra_params = num_outputs
        # else:
        #     print('WARNING: using hiddend_dim = 0 + multiplier * num_factors')
        #     extra_params = 0
        # multiplier <=0 -> linear probe
        # if self.multiplier <= 0:
        #     self.num_hidden_layers = 0
        ...
        if self.num_hidden_layers > 0:
            # TODO: should the hidden dim be proportional to num_factors?? or should if be fixed??
            # self.hidden_dim = extra_params + int(self.multiplier * self.num_factors)
        ...
        linear_probe = nn.Linear(self.hidden_dim, num_outputs)
        mlp_probe.append(linear_probe)
...
class IndependentMLPs(nn.Module):
    def __init__(self, num_factors, input_dim, num_hidden_layers=1, hidden_dim=0, factor_sizes=[2], 
        ...
        if self.num_hidden_layers > 0:
            ...
            #     list_norms.append(nn.LayerNorm([self.hidden_dim,1]))
            ...
            for i in range(self.num_hidden_layers - 1):
                ...
                if self.use_norm:
                    list_norms.append(nn.LayerNorm([self.hidden_dim,1], elementwise_affine=False))
                    # list_norms.append([nn.LayerNorm([self.hidden_dim,1]) for _ in range(num_factors) ] )
                    scale = torch.nn.Parameter(torch.Tensor(size=[self.hidden_dim * num_factors, 1]))
                    bias = torch.nn.Parameter(torch.Tensor(size=[self.hidden_dim * num_factors, 1]))
                    nn.init.constant_(bias, 0)
                    nn.init.constant_(scale, 1)
