...
from loss_capacity.utils import hsic_batch, hsic_batch_v2, hsic_batch_v2_comp, from_pickle, to_pickle
            ...
            for sigma in tqdm(sigma_hsic):
                ...
                for images, labels in dataloader: # dataloader is shuffled, we take a subsampling because it is too big.
                    while i < len(dataloader)/div_subsample:
                        ...
                        # v2
                        # hsic_score += hsic_batch_v2_comp(images, experiment, s_x=sigma*x_multi, s_y=sigma*y_multi, 
                        #                 device='cuda', num_sample_reparam=num_sample_reparam).item()
                        
                        # v2 comp
                        rand = torch.randn(images.shape)
                        hsic_score += hsic_batch_v2_comp(images, experiment, s_x=sigma*x_multi, s_y=sigma*y_multi, 
                                        device='cuda', num_sample_reparam=num_sample_reparam, compare = comp).item()
                        
                        i += 1
                hsic_score /= i
                results_hsic[str(sigma)] = hsic_score
                print(sigma, 'hsic_score testset', hsic_score)
            to_pickle(results_hsic, subfolder+'/hsic_num_reparam_'+str(num_sample_reparam)+'_'+ 'div_'+ str(div_subsample) + title_text_x +'_'+ title_text_y + '_'+ choose_dataloader +'_v3_comp_'+comp+'.pickle')
