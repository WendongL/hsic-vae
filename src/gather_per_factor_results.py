...
## mlp
    ...
    plot_name = f'individual_log_loss_mlp_uai_excess_{eps_params}' 

    capacity_measure = f'Total #Params - Linear #Params + {eps_params}'
    # folders = glob.glob("./results/results_plot1/*/*/*")
    # folders = glob.glob("./results/results_plot1/*beta*/*/*")
...
for folder in folders:
    ...
    if os.path.exists(piclkle_name):
        ...
        if model_txt not in all_r_results[dataset][probe_type]:
            all_r_results[dataset][probe_type][model_txt] = [(num_probe_params // div_capacity, 1.0 - val_rsquared_acc, 1.0 - test_rsquared_acc, mse, dci_scores)]
        else:
            all_r_results[dataset][probe_type][model_txt].append((num_probe_params // div_capacity, 1.0 - val_rsquared_acc, 1.0 - test_rsquared_acc, mse, dci_scores))
...
for dataset in all_r_results.keys():
    ...
    for probe_type in all_r_results[dataset].keys():
        ...
        fig_r, axs_r = plt.subplots(1,sharex=True, sharey=True)
        ...
        for model in sorted(all_r_results[dataset][probe_type].keys()):
            ...
            for x, runs_scores in dvals.items():
                ...
                std_test_scores = 1.96 * runs_scores[:,test_index,:].mean(-1).std()
                ...
                if ddci is not None and x in ddci:
                    ...
                    # ±1.96
                    std_c = 1.96 * np.array(all_c).std()
                    std_d = 1.96 * np.array(all_d).std()
            ...
            # axs_r.set_title('R squared Scores')
