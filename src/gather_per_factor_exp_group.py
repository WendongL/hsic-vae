...
dataset = 'mpi3d' #  'mpi3d' /  'cars3d' / 'dsprites'
plot_probe =  'mlp' # 'mlp' / 'rf' / 'rff'
plot_errorbars = True
explitcitness_mode='global' #'baseline' / 'global'

std_type = 'std1' # 'conf95' /  'std1'
plot_name = ''
### MPI3D
# MLP
...
if dataset == 'mpi3d':
    if plot_probe == 'mlp':
        ...
        # path_folders = '/home/anicolicioiu/data/models/results_uai/results_plot1_ind/vaes_ind/*/*'
        # path_folders = '/home/anicolicioiu/data/models/results_uai/results_plot1/*beta*/*/*'
        use_zoombox = True
        zoom_x1, zoom_x2 = 15.5, 16.8  # 0.97, 0.995
        zoom_y1, zoom_y2 = -0.009, 0.025
        zoom_bbox = (300, 170)
        zoom_val = 3.2
    # elif plot_probe == 'mlp_vaes':
        # VAES beta
        path_folders = '/home/anicolicioiu/data/models/results_mpi3d/results_vaes_betas/*/*/*'
        use_zoombox = False
        zoom_x1, zoom_x2 = 15.5, 16.8  # 0.97, 0.995
        zoom_y1, zoom_y2 = -0.009, 0.025
        zoom_bbox = (300, 170)
        zoom_val = 3.2
        plot_name = 'vae_betas_'

        # dataset size
        size=100 # 100 1000 5000 10000
        path_folders = f'/home/anicolicioiu/data/models/results_mpi3d/results_small_data/*/*data_fraction_{size}_*/*'
        use_zoombox = False
        plot_name = f'dataset_size_{size}_'
        ...
        # path_folders = '/home/anicolicioiu/data/models/results_rff/results_rff_0hid_all/*/*/*'
        path_folders = '/home/anicolicioiu/data/models/results_rff/results_rff_0hid_const/*/*/*'
        
        zoom_x1, zoom_x2 = 11.0, 12.0  # 0.97, 0.995
        zoom_y1, zoom_y2 = 0.00, 0.06
        zoom_bbox = (270, 170)
        zoom_val = 2.8
    ...
    if plot_probe == 'mlp':
        ...
        use_zoombox = True
        zoom_x1, zoom_x2 = 14.0, 15.9  # 0.97, 0.995
        zoom_y1, zoom_y2 = 0.03, 0.09
        zoom_bbox = (270, 115)
        zoom_val = 2.3
    elif plot_probe == 'rf':
        ...
        path_folders = '/home/anicolicioiu/data/models/results_cars3d/results_cars_rff_0hid/*/*/*'
        use_zoombox = True
        zoom_x1, zoom_x2 = 11.0, 12.0  # 0.97, 0.995
        zoom_y1, zoom_y2 = 0.06, 0.16
        zoom_bbox = (270, 170)
        zoom_val = 2.1
...
if plot_probe == 'rf':
    ...
    bottom=-0.02
    top = 0.8
    plot_name = plot_name + f'__3loss_rf_uai_excess' 
    ...
    plot_name = plot_name + f'individual_{plot_type}_loss_mlp_{eps_params}' 
    ...
    my_plot_title = 'Random Fourier Features (RFF) Probes'
    div_capacity = 7
    use_predefined_colors = True
    # use_predefined_colors = False
    ...
    # plot_name = f'per_factor_vae_log_loss_mlp_uai_excess_{eps_params}' 
    # plot_name = f'per_factor_log_loss_mlp_uai_excess_{eps_params}' 
    plot_name = plot_name + f'individual_{plot_type}_loss_RFF_linear2_normal_{eps_params}' 

    # capacity_measure = f'log(Total #Params - Linear #Params + {eps_params})'
    capacity_measure = f'log(RFF dimension)'
...
if True:
    ...
    for folder in folders:
        ...
        if os.path.exists(piclkle_name):
            ...
            probe_capacity = results['num_params']
            if plot_probe == 'rff':
                probe_capacity = results['capacity']

            if plot_probe == 'rf':
                if probe_capacity > 60:
            ...
            # if model_type == 'vae':
            #     if dci_scores != None and dci_scores != -1:
            #         print(f'{folder} DCI: {dci_scores} ')
            if dci_scores == -1:
                dci_scores = None
            # dci_scores = [
            ...
            if 'vae' in model_type:
                ...
                if results['params'].vae.exp_params.kld_weight == 0.0:
                    model_txt = 'AE'
                ...
                # if results['params'].vae.exp_params.kld_weight not in [1.0, 100]:
                    # continue
                if results['params'].vae.exp_params.kld_weight > 101:
                    continue
            if not isinstance(val_rsquared_acc, np.ndarray):
            ...
            if val_rsquared_acc.mean() < -0.1 or test_rsquared_acc.mean() < -0.1 :
                ...
                continue
            if plot_probe == 'mlp':
                probe_capacity = probe_capacity // div_capacity
                # probe_capacity = np.log(probe_capacity)
            
            exp_hash = [f'{key}:{val}__' for key,val in results['params'].probe.__dict__.items()]
            exp_hash = ''.join(exp_hash)

            if model_txt not in all_r_results[dataset][probe_type]:
                 all_r_results[dataset][probe_type][model_txt] = {}

            if probe_capacity not in all_r_results[dataset][probe_type][model_txt]:
                 all_r_results[dataset][probe_type][model_txt][probe_capacity] = {}
            
            if exp_hash not in all_r_results[dataset][probe_type][model_txt][probe_capacity]:
                 all_r_results[dataset][probe_type][model_txt][probe_capacity][exp_hash] = []


            # if model_txt not in all_r_results[dataset][probe_type]:

            #     all_r_results[dataset][probe_type][model_txt] = [(probe_capacity, 1.0 - val_rsquared_acc, 1.0 - test_rsquared_acc, mse, dci_scores)]
            # else:

            all_r_results[dataset][probe_type][model_txt][probe_capacity][exp_hash].append((1.0 - val_rsquared_acc, 1.0 - test_rsquared_acc, mse, dci_scores, folder))

            # print(folder)
            print(f'{id}  mse: {mse} R: {test_rsquared_acc} dci : {dci_scores}')

            # if model_txt == 'vae' and probe_capacity == 65536:
            #     pdb.set_trace()
...
for dataset in all_r_results.keys():
    per_fact_exp = [Explitcitness(mode=explitcitness_mode) for _ in range(num_factors)]
    Exp = Explitcitness(mode=explitcitness_mode)


    for probe_type in all_r_results[dataset].keys():

        key = next(iter(all_r_results[dataset][probe_type]))
        key2 = next(iter(all_r_results[dataset][probe_type][key]))
        key3 = next(iter(all_r_results[dataset][probe_type][key][key2]))

        num_factors = all_r_results[dataset][probe_type][key][key2][key3][0][1].shape[-1]
        ...
        if use_zoombox:
            ...
            axins = zoomed_inset_axes(axs_r, zoom_val, loc='center',  bbox_to_anchor=zoom_bbox)  # zoom = 9 # 
        ...
        for model in sorted(all_r_results[dataset][probe_type].keys()):
            ...
            dvals = {}
            ddci = {}
            # for capacity, c_res in all_r_results[dataset][probe_type][model].items():
            for capacity in sorted(all_r_results[dataset][probe_type][model].keys()):
                c_res = all_r_results[dataset][probe_type][model][capacity]
                all_groups_scores = []
                all_groups_names = []

                for group_hash, res in c_res.items():
                    # each group hash contains K seed results
                    # we take the mean results of all seeds and select only the best group
                    
                    # first take the mean over the factors then mean over the seeds
                    fact_mean = [r[0].mean() for r in res]
                    seed_mean = np.array(fact_mean).mean()
                    all_groups_scores.append(seed_mean)
                    all_groups_names.append(group_hash)
                
                best_group = np.argmin(np.array(all_groups_scores))
                best_group_key = all_groups_names[best_group]


                best_results_seeds = all_r_results[dataset][probe_type][model][capacity][best_group_key]
                dvals_unsorted[capacity] = [np.stack(seed[:2]) for seed in  best_results_seeds]
                ddci_unsorted[capacity] = [seed[-2] for seed in  best_results_seeds]
                [print(f'[{capacity}] Best group: {seed[-1]}') for seed in  best_results_seeds]    

            for cap in sorted(dvals_unsorted.keys()):
                dvals[cap] = dvals_unsorted[cap]
                ddci[cap] = ddci_unsorted[cap]


            #     mean_score_group = np.array(mean_score_group).mean()
            #     all_groups_scores.append(mean_score_group)
            # best_group = np.argmin(np.array(all_groups_scores))
            ...
            for x, runs_scores in dvals.items():
                ...
                if ddci is not None and x in ddci and (None not in ddci[x]):
            ...
            if use_predefined_colors:
                ...
                if not plot_errorbars:
                    ...
                    if use_zoombox:
                        ...
                        axins.set_ylim(bottom=0.0,top=1.0)
                    
                    axs_r.set_ylim(bottom=0.0,top=1.0)
        ...
        for f_i in range(num_factors):
            all_E = per_fact_exp[f_i].get_explitcitness(debug=False)
